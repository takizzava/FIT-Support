function dataList(payload) {
  ...
  return [];function dataList(payload) {
  if (Array.isArray(payload)) return payload.filter((x) => x && typeof x === "object");
  if (!payload || typeof payload !== "object") return [];
  if (Array.isArray(payload.data)) return payload.data.filter((x) => x && typeof x === "object");
  if (payload.data && typeof payload.data === "object") return [payload.data];
...
export class Chat2DeskAPI {
  ...
  ticketTitle(row) {
    const value = first(row, [
      ...
      "subject", "theme", "caption"
    ], null);
    if (value !== null && value !== undefined && String(value).trim()) return String(value).trim();
    const nested = row?.ticket && typeof row.ticket === "object"
      ? first(row.ticket, ["title", "ticket_title", "ticketTitle", "name", "ticket_name", "ticketName", "subject", "theme", "caption"], null)
      : null;
    return nested !== null && nested !== undefined && String(nested).trim() ? String(nested).trim() : "Без названия";
  }
